...
class RAGService {
  constructor() {
    const mode = process.env.QDRANT_MODE || 'local';
    this.isUsingCloud = mode === 'cloud';

    // Set collection name based on mode
    this.collectionName = this.isUsingCloud
      ? process.env.QDRANT_CLOUD_COLLECTION || 'pulmo'
      : process.env.QDRANT_LOCAL_COLLECTION || 'pulmo';
    ...
    // Configure Qdrant based on mode
    const qdrantConfig = this.isUsingCloud
      ? {
      ...
      : {
        url: process.env.QDRANT_LOCAL_URL,
        collectionName: this.collectionName,
      };

    this.vectorStore = new QdrantVectorStore(this.embeddings, qdrantConfig);

    // Log configuration
    logger.info(`Initializing Qdrant in ${mode} mode`);
    logger.info(`Using collection: ${this.collectionName}`);
    logger.info(`Qdrant URL: ${qdrantConfig.url}`);

    // Ensure collection exists on startup
    this.ensureCollectionExists(this.collectionName).catch(err => {
      logger.error(`Failed to initialize collection: ${err.message}`);
    });
      ...
      try {
        await this.vectorStore.client.createCollection(collectionName, {
          vectors: {
            size: vectorSize,
            distance: 'Cosine',
            on_disk: true // Enable on-disk storage for larger datasets
          },
          optimizers_config: {
